...
namespace Akiyama.IPC.Shared.Network.Packets
{
    ...
    public class TestPacket : Packet
    {
        ...
        public override void Populate()
        {
            ...
            byte[] strLenBytes = this.Payload.Take(4).ToArray(); // Here we grab the first 4 bytes of our data, since we know that the first 4 bytes are the length of the following string

            int strLen = PacketConstructor.BytesToInt32(strLenBytes); // Convert the bytes into an integer

            byte[] strBytes = this.Payload.Skip(4).Take(strLen).ToArray(); // Here we grab all of the bytes for our string, we use Skip(4) as we want to skip over the 4 bytes we already read for the string's length
            ...
            byte[] intBytes = this.Payload.Skip(4 + strLen).Take(4).ToArray(); // Here we grab the bytes for our final integer, skipping over the integer for our string's length, as well as the length of the string itself
        ...
        public override void Prepare()
        {
            ...
            this.SetPayload(data); // Finally, set this packet's data!
